...
app.controller('PortfolioController', function($scope, $compile, PortfolioFactory) {
    ...
    $scope.showCode = function(code, type, $event) {
        ...
        PortfolioFactory.getFileAsText('/code/'+code+'/'+code+'.js', function(data) {
            ...
            PortfolioFactory.getFileAsText('/code/'+code+'/'+type+'.js', function(data) {
                if (data) {
                    var html = $('<span ng-click="tabClick($event)">'+type+'.js</span>');
                    $('.code .tabs').append(html);
                    $('.code .details').append('<div class="detail">'+data+'</div>');
                    $compile(html)($scope);
                    count++;
                ...
                PortfolioFactory.getFileAsText('/code/'+code+'/styles/sass/app.scss', function(data) {
                    if (data) {
                    ...
                    PortfolioFactory.getFileAsText('/code/'+code+'/'+type+'.html', function(data) {
                        if (data) {
                            ...
                            $('.code .tabs').append(html);
                            $('.code .details').append('<div class="detail"><textarea>'+data+'</textarea></div>');
                            $compile(html)($scope)
                            count++;

                            var html = $('<span ng-click="tabClick($event)">Preview</span>'),
